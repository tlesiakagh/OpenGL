...
float randomFromRange(int maxValue, int option) {
	random_device rd; 
	mt19937 gen(rd());
	uniform_real_distribution<float> distNonSymmetric(0, maxValue); 
	uniform_real_distribution<float> distSymmetric(-maxValue, maxValue);
...
void sampleDiamond(int x, int y, int stepSize, float value)
{
	...
	//cout << "(" << x << ", " << y << "), " << "(" << x + hs << ", " << y + hs << "): " << d << endl;
	//cout << "(" << x << ", " << y << "), " << "(" << x + hs << ", " << y - hs << "): " << c << endl;
	//cout << "DiamondValue: " << ((a + b + c + d) / 4.0) + value << " at pos (" << y << ", " << x << ")" << endl;
